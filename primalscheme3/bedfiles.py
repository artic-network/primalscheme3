...
def calc_median_bed_tm(primers: list[list[BedLine]], cfg) -> tuple[float, float, float]:
    """
    Given a list of BedLines in pools, generated by parse_bedfile()
...
def read_in_bedfile(path: pathlib.Path) -> list[BedPrimerPair]:
    ...
    for ref in {bedline.ref for bedline in bed_lines}:
        ref_to_bedlines[ref]: list[BedLine] = [x for x in bed_lines if x.ref == ref]
