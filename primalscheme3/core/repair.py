...
def thermo_check_seq(seq, base_cfg: Config) -> THERMORESULT | DIGESTION_ERROR:
    # Thermo check
    ...
    # Check for Homodimer
    if do_pools_interact_py([seq], [seq], base_cfg.dimer_score):
...
def repair(
    ...
    for pp in primerpairs_in_msa:
        ...
        _start_col, rseq_counts = r_digest_index(
            msa.array, base_cfg, msa_rkmer_start, base_cfg["minbasefreq"]
        )
        assert isinstance(rseq_counts, dict)
        # Check for early return conditions
