...
def amplicon_primer_gff_rows(
    list_primer_pair: list[PrimerPair], cfg: dict
) -> list[list[tuple[str, ...]]]:
...
def write_report(list_pp, OUTPUT_DIR, cfg, reference_fasta) -> None:
    """
    Write HTML report and associated JSON

    reference_fasta: The first Genome in the msa as .format("fasta")
    """
    # Data JSON
    data = {
        "primerTracks": [
            get_data_uri(track)
            for track in amplicon_primer_gff(amplicon_primer_gff_rows(list_pp, cfg))
        ],
    }
